...
# Canonical public origin of this deployment (MUST be externally reachable for Claude Web)
MCP_SERVER_URL = os.getenv("MCP_SERVER_URL", "http://localhost:8000").rstrip("/")
OAUTH_ISSUER = os.getenv("OAUTH_ISSUER", MCP_SERVER_URL).rstrip("/")
OAUTH_INTROSPECTION_URL = os.getenv("OAUTH_INTROSPECTION_URL", f"{MCP_SERVER_URL}/introspect")
...
def protected_resource_metadata_url_for(resource_url: str) -> str:
    """
    RFC 9728 §3.1: if the resource has a path, insert /.well-known/oauth-protected-resource
    ...
    """
    # resource_url is expected to be absolute and to share origin with MCP_SERVER_URL
    # We build from MCP_SERVER_URL to avoid subtle proxy/host header issues.
...
def get_www_authenticate_header(
    ...
    # Add resource metadata URL (RFC 9728)
    res = (resource_url or f"{MCP_SERVER_URL}/v2/mcp").rstrip("/")
    metadata_url = protected_resource_metadata_url_for(res)
